...
// ====================== نظام الصلاحيات: مدير المعهد / مدرس ======================
// أي أدمن قديم بدون role (أو role = 'admin') يُعامل كمدير معهد لضمان التوافق مع الحسابات الحالية
function getAdminRole() { const u = getLoggedInUser(); return (u && u.role === 'teacher') ? 'teacher' : 'manager'; }
function isManagerRole() { return getAdminRole() === 'manager'; }
function roleLabel(role) { return role === 'teacher' ? '👨‍🏫 مدرس' : '👑 مدير المعهد'; }
// إخفاء الأقسام المخصصة لمدير المعهد فقط عن حساب المدرس
function applyRolePermissions() {
    const isMgr = isManagerRole();
    document.querySelectorAll('.manager-only').forEach(el => { el.style.display = isMgr ? '' : 'none'; });
}
