// admin.js - النسخة الكاملة النهائية (جميع الوظائف + تحليل Excel لايف + دين في النتائج)
...
function normalizeSubjectName(name) {
    if (!name) return ''; const m = { 'التربية الدينية': 'الدين', 'تربية دينية': 'الدين', 'دين': 'الدين', 'الكمبيوتر': 'حاسب آلي', 'كمبيوتر': 'حاسب آلي', 'الحاسب الآلي': 'حاسب آلي', 'الفيزياء': 'فيزياء', 'الكيمياء': 'كيمياء', 'التمريض الباطني الجراحي': 'تمريض باطني جراحي', 'تمريض باطنى جراحي': 'تمريض باطني جراحي', 'الطب الباطنة': 'طب باطنة', 'العلوم التطبيقية': 'علوم تطبيقية', 'الصحة المجتمع': 'صحة مجتمع', 'الجراحة العامة': 'جراحة عامة' };
...
function calculateStudentTotal(st) { if (!st.subjects) return 0; const cfg = getSubjectConfig(st.semester || 'first'); let t = 0; st.subjects.forEach(s => { const n = normalizeSubjectName(s.name); const c = cfg[n]; if (c && !c.isExtra) t += s.grade || 0; }); return t; }
function calculateStudentPercentage(st) { const t = calculateStudentTotal(st); const p = getTotalPossible(st.semester || 'first'); return p > 0 ? (t / p) * 100 : 0; }
function getStudentFormattedGrades(st) { const sem = st.semester || 'first'; const cfg = getSubjectConfig(sem); const ord = getOrderedSubjects(sem); let g = {}; ord.forEach(n => { const c = cfg[n]; const sub = st.subjects?.find(s => normalizeSubjectName(s.name) === n); g[n] = { grade: sub?.grade || 0, max: c.max, isExtra: c.isExtra || false }; }); return g; }
...
window.addNotification = async function() {
    const text = document.getElementById('notification-text')?.value.trim(); if (!text) { showToast('يرجى إدخال نص الإشعار!', 'error'); return; }
    const date = new Date().toLocaleString('ar-EG'); showToast('جاري إضافة الإشعار...', 'info');
    try {
        const csrfToken = getCsrfToken(); const response = await fetch(`${BASE_URL}/api/notifications`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrfToken }, credentials: 'include', body: JSON.stringify({ text, date }) });
        const data = await response.json(); if (response.ok && data.success) { await loadNotifications(); document.getElementById('notification-text').value = ''; showToast('✅ تم إضافة الإشعار بنجاح!', 'success'); } else { showToast(data.error || 'فشل إضافة الإشعار', 'error'); }
    } catch (error) { showToast('حدث خطأ أثناء إضافة الإشعار', 'error'); }
};

window.deleteNotification = async function(id) {
    const result = await Swal.fire({ title: '⚠️ تأكيد الحذف', text: 'هل أنت متأكد من حذف هذا الإشعار؟', icon: 'warning', showCancelButton: true, confirmButtonText: 'نعم، احذف', cancelButtonText: 'إلغاء', confirmButtonColor: '#E74C3C' });
    if (result.isConfirmed) { try { const csrfToken = getCsrfToken(); const response = await fetch(`${BASE_URL}/api/notifications/${id}`, { method: 'DELETE', headers: { 'X-CSRF-Token': csrfToken, 'Content-Type': 'application/json' }, credentials: 'include' }); const data = await response.json(); if (response.ok && data.success) { await loadNotifications(); showToast('🗑️ تم حذف الإشعار بنجاح.', 'success'); } else { showToast(data.error || '❌ فشل حذف الإشعار', 'error'); } } catch (error) { showToast('حدث خطأ أثناء حذف الإشعار', 'error'); } }
};
...
function renderStats() {
    ...
    const sem = topStd?.semester || 'first', tp = getTotalPossible(sem);
    sec.innerHTML = `<div class="stats-grid"><div class="stat-item"><i class="fas fa-users"></i> عدد الطلاب: ${total}</div><div class="stat-item"><i class="fas fa-chart-line"></i> المتوسط: ${avg}%</div><div class="stat-item"><i class="fas fa-check-circle"></i> الناجحين: ${passed}</div><div class="stat-item"><i class="fas fa-times-circle"></i> الراسبين: ${total - passed}</div></div>${topStd?`<div class="stats-grid" style="margin-top:15px;background:linear-gradient(135deg,#C7A252,#A07D3A);border-radius:15px;padding:15px;"><div class="stat-item" style="text-align:center;background:none;"><i class="fas fa-trophy" style="font-size:2rem;color:#fff;"></i><p style="font-weight:bold;">🏆 أعلى طالب</p><p>${escapeHtml(topStd.fullName)}</p><p>رقم الجلوس: ${topStd.studentCode}</p><p>المجموع: ${calculateStudentTotal(topStd)} / ${tp}</p><p>النسبة: ${topPct.toFixed(1)}%</p></div></div>`:''}`;
}
...
function renderResults(filter = '') {
    ...
    filtered.forEach(st => {
        const sem = st.semester || 'first', total = calculateStudentTotal(st), pct = calculateStudentPercentage(st);
        const grades = getStudentFormattedGrades(st), ord = getOrderedSubjects(sem), tp = getTotalPossible(sem);
        let html = '<div class="subjects-container">';
        for (let n of ord) { let gi = grades[n]; if (gi) { if (gi.isExtra) html += `<div class="extra-subject">📖 ${n}: <strong>${gi.grade}</strong> / ${gi.max} <small>(خارج المجموع)</small></div>`; else html += `<div class="subject-row"><span class="subject-name"><i class="fas fa-book"></i> ${n}</span><span class="subject-grade">${gi.grade} / ${gi.max}</span></div>`; } }
        ...
        let row = tbody.insertRow(); row.innerHTML = `<td><strong>${escapeHtml(st.fullName)}</strong><br><small>رقم الجلوس: ${st.studentCode}</small><br><small>الترم: ${sem==='first'?'الأول':'الثاني'}</small></td><td>${html}</td><td><span class="total-cell">${total} / ${tp}</span></td><td><span class="percentage-cell ${pClass}">${pct.toFixed(1)}% (${pText})</span></td><td><button class="table-action-btn edit-action" onclick="editStudent('${st.studentCode}')"><i class="fas fa-edit"></i></button> <button class="table-action-btn delete-action" onclick="deleteStudent('${st.studentCode}')"><i class="fas fa-trash"></i></button></td>`;
...
window.editStudent = (code) => { let s=allStudents.find(st=>st.studentCode===code); if(s){ document.getElementById('student-name').value=s.fullName; document.getElementById('student-id').value=s.studentCode; document.getElementById('semester').value=s.semester||'first'; const allInputs=document.querySelectorAll('#add-result-form input[type="number"]'); allInputs.forEach(inp=>inp.value=0); window.toggleSubjects(); s.subjects?.forEach(sub=>{ const nn=normalizeSubjectName(sub.name); const sem=s.semester||'first'; if(sem==='first'){ switch(nn){ case'اللغة العربية':document.getElementById('subject2').value=sub.grade;break; case'اللغة الإنجليزية':document.getElementById('subject3').value=sub.grade;break; case'علوم تطبيقية':document.getElementById('subject4').value=sub.grade;break; case'طب باطنة':document.getElementById('subject5').value=sub.grade;break; case'تمريض باطني جراحي':document.getElementById('subject6').value=sub.grade;break; case'حاسب آلي':document.getElementById('subject8').value=sub.grade;break; case'الدين':document.getElementById('subject7').value=sub.grade;break; } } else { switch(nn){ case'اللغة العربية':const a2=document.getElementById('subject2-second');if(a2)a2.value=sub.grade;break; case'اللغة الإنجليزية':const e2=document.getElementById('subject3-second');if(e2)e2.value=sub.grade;break; case'تمريض باطني جراحي':const n2=document.getElementById('subject6-second');if(n2)n2.value=sub.grade;break; case'صحة مجتمع':document.getElementById('subject1').value=sub.grade;break; case'جراحة عامة':const s2=document.getElementById('subject4-second');if(s2)s2.value=sub.grade;break; case'حاسب آلي':const c2=document.getElementById('subject8-second');if(c2)c2.value=sub.grade;break; case'الإحصاء':const st2=document.getElementById('subject5-second');if(st2)st2.value=sub.grade;break; } } }); showToast('✏️ قم بتعديل البيانات ثم اضغط حفظ','info'); window.scrollTo(0,0); } };
...
document.getElementById('add-result-form')?.addEventListener('submit', async (e) => { 
    ...
    let subjects=[]; 
    if(sem==='first'){ subjects=[{name:"اللغة العربية",grade:parseInt(document.getElementById('subject2').value)||0},{name:"اللغة الإنجليزية",grade:parseInt(document.getElementById('subject3').value)||0},{name:"علوم تطبيقية",grade:parseInt(document.getElementById('subject4').value)||0},{name:"طب باطنة",grade:parseInt(document.getElementById('subject5').value)||0},{name:"تمريض باطني جراحي",grade:parseInt(document.getElementById('subject6').value)||0},{name:"حاسب آلي",grade:parseInt(document.getElementById('subject8').value)||0},{name:"الدين",grade:parseInt(document.getElementById('subject7').value)||0}]; } 
    else { const s2=document.getElementById('subject2-second'),s3=document.getElementById('subject3-second'),s6=document.getElementById('subject6-second'),s4=document.getElementById('subject4-second'),s8=document.getElementById('subject8-second'),s5=document.getElementById('subject5-second'); subjects=[{name:"اللغة العربية",grade:s2?parseInt(s2.value)||0:0},{name:"اللغة الإنجليزية",grade:s3?parseInt(s3.value)||0:0},{name:"تمريض باطني جراحي",grade:s6?parseInt(s6.value)||0:0},{name:"صحة مجتمع",grade:parseInt(document.getElementById('subject1').value)||0},{name:"جراحة عامة",grade:s4?parseInt(s4.value)||0:0},{name:"حاسب آلي",grade:s8?parseInt(s8.value)||0:0},{name:"الإحصاء",grade:s5?parseInt(s5.value)||0:0}]; } 
...
window.saveExam = async function() { const name=document.getElementById('exam-name')?.value.trim(), code=document.getElementById('exam-code')?.value.trim(), stage=document.getElementById('exam-stage')?.value, duration=parseInt(document.getElementById('exam-duration')?.value); if(!name){ showToast('يرجى إدخال اسم الاختبار!','error'); return; } if(!code){ showToast('يرجى إدخال كود الاختبار!','error'); return; } if(!duration||duration<1){ showToast('يرجى إدخال مدة الاختبار بالدقائق!','error'); return; } if(questionsList.length===0){ showToast('يرجى إضافة سؤال واحد على الأقل!','error'); return; } const csrfToken=getCsrfToken(); showToast('جاري حفظ الاختبار...','info'); try { const response=await fetch(`${BASE_URL}/api/exams`,{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken},credentials:'include',body:JSON.stringify({name,code,stage,duration,questions:questionsList})}); const data=await response.json(); if(response.ok&&data.success){ showToast('✅ تم حفظ الاختبار بنجاح!','success'); questionsList=[]; document.getElementById('questions-list').innerHTML=''; document.getElementById('exam-name').value=''; document.getElementById('exam-code').value=''; document.getElementById('exam-duration').value=''; document.getElementById('exam-stage').value='first'; document.getElementById('question-inputs').innerHTML=''; await loadExamsList(); } else { showToast(data.error||'❌ فشل حفظ الاختبار','error'); } } catch(error){ showToast('❌ خطأ في الاتصال بالسيرفر','error'); } };
async function loadExamsList() { try { const response=await fetch(`${BASE_URL}/api/exams`,{credentials:'include'}); if(!response.ok) throw new Error('فشل جلب الاختبارات'); const exams=await response.json(); const tbody=document.getElementById('exams-list-body'); if(!tbody) return; if(!exams||exams.length===0){ tbody.innerHTML='<tr><td colspan="7">📭 لا توجد اختبارات مسجلة</td></tr>'; return; } tbody.innerHTML=exams.map(ex=>`<tr><td>${escapeHtml(ex.name)}</td><td><strong style="color:#0F2B3D;">${escapeHtml(ex.code)}</strong></td><td>${ex.stage==='first'?'الأولى ثانوي':'الثانية ثانوي'}</td><td>${ex.duration} دقيقة</td><td>${ex.questions?.length||0}</td><td>${new Date(ex.createdAt).toLocaleDateString('ar-EG')}</td><td><button class="edit-btn" onclick="viewExam('${ex.code}')" style="background:#3498DB;color:white;border:none;padding:6px 12px;border-radius:20px;margin:2px;cursor:pointer;"><i class="fas fa-eye"></i> عرض</button> <button class="delete-btn" onclick="deleteExam('${ex.code}')" style="background:#E74C3C;color:white;border:none;padding:6px 12px;border-radius:20px;margin:2px;cursor:pointer;"><i class="fas fa-trash"></i> حذف</button></td></tr>`).join(''); } catch(error){ const tbody=document.getElementById('exams-list-body'); if(tbody) tbody.innerHTML='<tr><td colspan="7">❌ فشل تحميل الاختبارات</td></tr>'; } }
window.viewExam = async function(code) { try { const response=await fetch(`${BASE_URL}/api/exams/${encodeURIComponent(code)}`,{credentials:'include'}); if(!response.ok) throw new Error('فشل جلب بيانات الاختبار'); const exam=await response.json(); let qh='<div style="max-height:400px;overflow-y:auto;text-align:right;">'; exam.questions.forEach((q,i)=>{ qh+=`<div style="background:#f8f9fa;border-radius:12px;padding:15px;margin-bottom:15px;"><strong>سؤال ${i+1}:</strong> ${escapeHtml(q.text)}<br>${q.options?`<span style="color:#2D9C7C;">📌 الخيارات: ${q.options.join(', ')}</span><br><span style="color:#C7A252;">✅ الإجابة الصحيحة: ${q.correctAnswer==='true'?'صح':(q.correctAnswer==='false'?'خطأ':q.correctAnswer)}</span>`:''}${q.correctAnswer&&!q.options?`<span style="color:#C7A252;">✅ الإجابة النموذجية: ${escapeHtml(q.correctAnswer)}</span>`:''}</div>`; }); qh+='</div>'; Swal.fire({title:exam.name,html:`<div style="text-align:right;"><p><strong>🔑 كود الاختبار:</strong> ${exam.code}</p><p><strong>📚 المرحلة:</strong> ${exam.stage==='first'?'الأولى ثانوي':'الثانية ثانوي'}</p><p><strong>⏱️ المدة:</strong> ${exam.duration} دقيقة</p><p><strong>📊 عدد الأسئلة:</strong> ${exam.questions.length}</p><hr><h4>📝 الأسئلة:</h4>${qh}</div>`,icon:'info',confirmButtonText:'إغلاق',confirmButtonColor:'#C7A252',width:'700px'}); } catch(error){ showToast('خطأ في عرض بيانات الاختبار','error'); } };
window.deleteExam = async function(code) { const result=await Swal.fire({title:'⚠️ تأكيد الحذف',text:`هل أنت متأكد من حذف الاختبار "${code}"؟ لا يمكن التراجع!`,icon:'warning',showCancelButton:true,confirmButtonText:'نعم، احذف',cancelButtonText:'إلغاء',confirmButtonColor:'#E74C3C'}); if(result.isConfirmed){ const csrfToken=getCsrfToken(); showToast('جاري حذف الاختبار...','info'); try { const response=await fetch(`${BASE_URL}/api/exams/${encodeURIComponent(code)}`,{method:'DELETE',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken},credentials:'include'}); const data=await response.json(); if(response.ok&&data.success){ showToast('✅ تم حذف الاختبار بنجاح','success'); await loadExamsList(); } else { showToast(data.error||'❌ فشل حذف الاختبار','error'); } } catch(error){ showToast('❌ خطأ في الاتصال بالسيرفر','error'); } } };
document.getElementById('fetch-results')?.addEventListener('click', async () => { const code=document.getElementById('results-exam-code')?.value.trim(); if(!code){ showToast('أدخل كود الاختبار أولاً!','error'); return; } try { const response=await fetch(`${BASE_URL}/api/exams/${encodeURIComponent(code)}/results`,{credentials:'include'}); if(!response.ok) throw new Error('فشل جلب النتائج'); const results=await response.json(); const container=document.getElementById('exam-results-list'); if(!results||results.length===0){ container.innerHTML='<p style="text-align:center;padding:20px;">📭 لا توجد نتائج مسجلة لهذا الاختبار</p>'; return; } container.innerHTML=`<div class="table-wrapper"><table style="width:100%;border-collapse:collapse;"><thead><tr style="background:#1e3c4a;color:#d4af5a;"><th style="padding:12px;">👨‍🎓 الطالب</th><th style="padding:12px;">📊 النتيجة</th><th style="padding:12px;">📅 التاريخ</th></tr></thead><tbody>${results.map(r=>`<tr style="border-bottom:1px solid #eee;"><td style="padding:10px;text-align:center;">${escapeHtml(r.studentId)}</td><td style="padding:10px;text-align:center;"><strong style="color:#27AE60;">${r.score.toFixed(1)}%</strong></td><td style="padding:10px;text-align:center;">${new Date(r.completionTime).toLocaleString('ar-EG')}</td></tr>`).join('')}</tbody></table></div>`; } catch(error){ showToast('❌ خطأ في جلب النتائج','error'); } });
...
// ====================== ✅ تحليل Excel (بدون صف - اكتشاف تلقائي للترم من المواد) ======================
window.analyzeExcel = async () => { 
    ...
    reader.onload = async (e) => { 
        try {
            ...
            // ✅ اكتشاف الترم من أول صف فيه مواد
            let detectedSemester = 'first';
            for (let i = 1; i < rows.length; i++) {
                let row = rows[i];
                if (!row[0] || !row[1]) continue;
                // جمع أسماء المواد من الصف
                let subjectNames = [];
                if (row[2] !== undefined && row[2] !== '') subjectNames.push("اللغة العربية");
                if (row[3] !== undefined && row[3] !== '') subjectNames.push("اللغة الإنجليزية");
                if (row[4] !== undefined && row[4] !== '') subjectNames.push("علوم تطبيقية");
                if (row[5] !== undefined && row[5] !== '') subjectNames.push("طب باطنة");
                if (row[6] !== undefined && row[6] !== '') subjectNames.push("تمريض باطني جراحي");
                if (row[7] !== undefined && row[7] !== '') subjectNames.push("حاسب آلي");
                if (row[8] !== undefined && row[8] !== '') subjectNames.push("الدين");
                
                // لو لقينا مواد مميزة للترم التاني، يبقى ترم ثاني
                if (subjectNames.includes("صحة مجتمع") || subjectNames.includes("جراحة عامة") || subjectNames.includes("الإحصاء")) {
                    detectedSemester = 'second'; break;
                }
                // لو لقينا مواد مميزة للترم الأول، يبقى ترم أول
                if (subjectNames.includes("علوم تطبيقية") || subjectNames.includes("طب باطنة") || subjectNames.includes("الدين")) {
                    detectedSemester = 'first'; break;
                }
            }
            console.log(`📚 الترم المكتشف: ${detectedSemester === 'first' ? 'الترم الأول' : 'الترم الثاني'}`);
            ...
            progressContainer.innerHTML = `
                <div style="margin-bottom:10px;text-align:center;font-weight:bold;color:#1a4f6e;">
                    ...
                    📚 الترم: <span style="color:#c4a35a;">${detectedSemester === 'first' ? 'الأول' : 'الثاني'}</span>
                </div>
                ...
                <div id="progress-stats" style="text-align:center;margin-bottom:10px;font-weight:bold;">🔄 تحديث: <span style="color:#3498db;">0</span> | ➕ جديد: <span style="color:#8e44ad;">0</span> | ⏭️ تخطي: <span style="color:#f39c12;">0</span> | ❌ خطأ: <span style="color:#e74c3c;">0</span></div>
            ...
            function up(c, t) { const p = Math.round((c/t)*100); if(pb) pb.style.width = p+'%'; if(pt) pt.innerHTML = `⏳ جاري التحليل... ${c}/${t} (${p}%)`; if(pst) pst.innerHTML = `🔄 تحديث: <span style="color:#3498db;">${updatedCount}</span> | ➕ جديد: <span style="color:#8e44ad;">${addedCount}</span> | ⏭️ تخطي: <span style="color:#f39c12;">${skippedCount}</span> | ❌ خطأ: <span style="color:#e74c3c;">${errorCount}</span>`; }
            function log(msg, type) { const colors = { success: '#27ae60', warning: '#f39c12', error: '#e74c3c', add: '#8e44ad', update: '#2980b9' }, icons = { success: '✅', warning: '⏭️', error: '❌', add: '➕', update: '🔄' }; const time = new Date().toLocaleTimeString('ar-EG'); liveLog.unshift({ msg, type, time }); if (liveLog.length > 100) liveLog.pop(); if (ll) ll.innerHTML = liveLog.map(l => `<div style="padding:3px 0;border-bottom:1px solid #f0f0f0;color:${colors[l.type]};"><small>${l.time}</small> ${icons[l.type]} ${l.msg}</div>`).join(''); }
            
            // ✅ جمع كل البيانات وإرسالها مرة واحدة
            let studentsData = [];
            ...
            for (let i = 1; i < rows.length; i++) { 
                ...
                // ✅ قراءة كل الأعمدة وتحديد المواد حسب القيم الموجودة
                let subjects = [];
                
                if (detectedSemester === 'first') {
                    // الترم الأول: عربي، إنجليزي، علوم تطبيقية، طب باطنة، تمريض، حاسب آلي، الدين
                    ...
                    // الترم الثاني: عربي، إنجليزي، تمريض، صحة مجتمع، جراحة عامة، حاسب آلي، إحصاء
                    if (row[2] !== undefined && row[2] !== '') subjects.push({ name: "اللغة العربية", grade: parseFloat(row[2]) || 0 });
                    if (row[3] !== undefined && row[3] !== '') subjects.push({ name: "اللغة الإنجليزية", grade: parseFloat(row[3]) || 0 });
                    if (row[4] !== undefined && row[4] !== '') subjects.push({ name: "تمريض باطني جراحي", grade: parseFloat(row[4]) || 0 });
                    if (row[5] !== undefined && row[5] !== '') subjects.push({ name: "صحة مجتمع", grade: parseFloat(row[5]) || 0 });
                    if (row[6] !== undefined && row[6] !== '') subjects.push({ name: "جراحة عامة", grade: parseFloat(row[6]) || 0 });
                    if (row[7] !== undefined && row[7] !== '') subjects.push({ name: "حاسب آلي", grade: parseFloat(row[7]) || 0 });
                    if (row[8] !== undefined && row[8] !== '') subjects.push({ name: "الإحصاء", grade: parseFloat(row[8]) || 0 });
                }
                
                let existing = allStudents.find(s => s.studentCode == studentCode);
                
                if (existing) { updatedCount++; log(`تحديث: ${studentName} (${studentCode})`, 'update'); }
                else { addedCount++; log(`إضافة: ${studentName} (${studentCode})`, 'add'); }
            ...
            if (studentsData.length > 0) {
                try {
                    ...
                    log(`✅ تم إرسال ${studentsData.length} طالب للسيرفر`, 'success');
                } catch (err) {
                    errorCount++;
...
// ====================== تصدير Excel ======================
function exportToExcel() { if(!studentsWithGrades.length){ showToast('لا توجد بيانات','error'); return; } const data=studentsWithGrades.map(st=>{ const grades=getStudentFormattedGrades(st), sem=st.semester||'first', ord=getOrderedSubjects(sem); const row={'رقم الجلوس':st.studentCode,'اسم الطالب':st.fullName,'الترم':sem==='first'?'الأول':'الثاني'}; ord.forEach(s=>{ const gi=grades[s]; if(gi) row[s]=gi.grade; }); row['المجموع']=calculateStudentTotal(st); row['النسبة المئوية']=calculateStudentPercentage(st).toFixed(1)+'%'; return row; }); const ws=XLSX.utils.json_to_sheet(data), wb=XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb,ws,'النتائج'); XLSX.writeFile(wb,'نتائج_الطلاب.xlsx'); showToast('✅ تم تصدير النتائج بنجاح','success'); }
...
function renderTopStudents() { const c=document.getElementById('top-students-grid'); if(!c) return; topStudentsList=calculateTopStudents(); if(!topStudentsList.length){ c.innerHTML='<div style="text-align:center;padding:40px;">📭 لا توجد بيانات كافية لعرض الأوائل</div>'; return; } const medals=['🥇','🥈','🥉','📖','📚','🏅','⭐','🌟','✨','🎯']; c.innerHTML=topStudentsList.map((s,i)=>{ const rank=i+1, rc=rank===1?'rank-1':(rank===2?'rank-2':(rank===3?'rank-3':'')), medal=medals[i]||'📜'; let pc=s.percentage>=95?'excellent':(s.percentage>=85?'very-good':(s.percentage>=75?'good':'')); return `<div class="top-student-card ${rc}"><div class="top-student-rank">${medal} ${rank}</div><div class="top-student-avatar"><i class="fas fa-user-graduate"></i></div><div class="top-student-name">${escapeHtml(s.fullName)}</div><div class="top-student-code">📋 رقم الجلوس: ${s.studentCode}</div><div class="top-student-percentage"><span class="percentage-cell ${pc}">${s.percentage.toFixed(1)}%</span></div><div class="top-student-total">📊 المجموع: ${s.total} / ${getTotalPossible(s.semester||'first')}</div><button class="certificate-btn" style="margin-top:15px;padding:8px 20px;font-size:0.8rem;" onclick='showCertificate(${JSON.stringify(s).replace(/'/g,"&#39;")})'><i class="fas fa-award"></i> شهادة تقدير</button></div>`; }).join(''); }
window.refreshTopStudents = function() { showToast('جاري تحديث قائمة الأوائل...','info'); renderTopStudents(); showToast('✅ تم تحديث قائمة العشرة الأوائل','success'); };
window.showCertificate = function(student) { currentCertificateStudent=student; const container=document.getElementById('certificate-container'), modal=document.getElementById('certificate-modal'); if(!container||!modal) return; const currentDate=new Date().toLocaleDateString('ar-EG',{year:'numeric',month:'long',day:'numeric'}); const rank=topStudentsList.findIndex(s=>s.studentCode===student.studentCode)+1; const rankText=getRankText(rank); const sem=student.semester||'first', tp=getTotalPossible(sem); container.innerHTML=`<div class="certificate" id="certificate-to-print"><div class="certificate-logo"><img src="logo.png" alt="شعار المعهد" onerror="this.style.display='none'"></div><div class="certificate-title"><i class="fas fa-award"></i> شهادة تقدير</div><div class="certificate-subtitle">معهد رعاية الضبعية الفني للتمريض</div><div class="certificate-body"><p>تُمنح هذه الشهادة للطالب/ة</p><div class="certificate-student-name">${escapeHtml(student.fullName)}</div><p>رقم الجلوس: <strong>${student.studentCode}</strong></p><div class="certificate-rank">🏆 <span>${rankText}</span> 🏆</div><div class="certificate-percentage">⭐ بنسبة نجاح ${student.percentage.toFixed(1)}% ⭐</div><p>📊 المجموع الكلي: ${student.total} / ${tp}</p><p style="margin-top:15px;color:#666;">وذلك تقديراً لتفوقه وجهده المتميز خلال الفصل الدراسي،<br>ونتمنى له دوام النجاح والتفوق.</p></div><div class="certificate-date"><i class="far fa-calendar-alt"></i> التاريخ: ${currentDate}</div><div class="certificate-signature"><div><hr><p>مدير المعهد</p></div><div><hr><p>وكيل المعهد</p></div></div></div>`; modal.classList.add('show'); };
...
window.closeCertificateModal = function() { const modal=document.getElementById('certificate-modal'); if(modal) modal.classList.remove('show'); currentCertificateStudent=null; };
window.printCurrentCertificate = function() { const cc=document.getElementById('certificate-to-print'); if(!cc) return; const pw=window.open('','_blank'); pw.document.write(`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>شهادة تقدير - ${currentCertificateStudent?.fullName||'طالب'}</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tajawal',sans-serif;background:white;display:flex;justify-content:center;align-items:center;min-height:100vh;padding:20px}.certificate{background:linear-gradient(135deg,#fff 0%,#fdf8ed 100%);padding:40px;margin:20px;border:20px double #c4a35a;border-radius:28px;text-align:center;max-width:800px;width:100%}.certificate-logo img{width:80px;height:80px;border-radius:50%;border:3px solid #c4a35a}.certificate-title{font-size:2rem;font-weight:800;color:#1a4f6e;margin:20px 0}.certificate-subtitle{font-size:1rem;color:#64748b;margin-bottom:30px;border-bottom:2px solid #c4a35a;display:inline-block;padding-bottom:5px}.certificate-student-name{font-size:1.8rem;font-weight:800;background:linear-gradient(135deg,#c4a35a,#a07d3a);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin:20px 0}.certificate-rank span{font-size:1.5rem;font-weight:800;color:gold}.certificate-percentage{font-size:1.4rem;font-weight:800;color:#1a4f6e;margin:15px 0}.certificate-date{margin-top:30px;padding-top:20px;border-top:1px dashed #cbd5e1}.certificate-signature{margin-top:30px;display:flex;justify-content:space-between;align-items:center;padding:0 20px}.certificate-signature hr{width:150px;margin:5px 0;border:1px solid #cbd5e1}@media print{body{padding:0;margin:0}.certificate{margin:0;page-break-inside:avoid}}</style></head><body>${cc.outerHTML}<script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()};<\/script></body></html>`); pw.document.close(); };
window.downloadCurrentCertificate = async function() { if(!currentCertificateStudent) return; showToast('جاري إنشاء ملف PDF...','info'); if(typeof html2pdf!=='undefined'){ const e=document.getElementById('certificate-to-print'); html2pdf().set({margin:[0.5,0.5,0.5,0.5],filename:`شهادة_تقدير_${currentCertificateStudent.fullName}.pdf`,image:{type:'jpeg',quality:0.98},html2canvas:{scale:2,letterRendering:true},jsPDF:{unit:'in',format:'a4',orientation:'portrait'}}).from(e).save(); showToast('✅ تم حفظ الشهادة','success'); } else { showToast('⚠️ استخدم طباعة ثم حفظ كـ PDF','warning'); printCurrentCertificate(); } };
window.printAllCertificates = function() { if(!topStudentsList.length){ showToast('لا توجد بيانات','error'); return; } const pw=window.open('','_blank'); const cd=new Date().toLocaleDateString('ar-EG',{year:'numeric',month:'long',day:'numeric'}); let html=`<!DOCTYPE html><html dir="rtl"><head><meta charset="UTF-8"><title>شهادات تكريم العشرة الأوائل</title><link href="https://fonts.googleapis.com/css2?family=Tajawal:wght@400;500;700;800&display=swap" rel="stylesheet"><style>*{margin:0;padding:0;box-sizing:border-box}body{font-family:'Tajawal',sans-serif;background:#fff;padding:20px}.certificate-page{page-break-after:always;display:flex;justify-content:center;align-items:center;min-height:100vh}.certificate{background:linear-gradient(135deg,#fff 0%,#fdf8ed 100%);padding:40px;margin:20px;border:20px double #c4a35a;border-radius:28px;text-align:center;max-width:800px;width:100%}.certificate-logo img{width:80px;height:80px;border-radius:50%;border:3px solid #c4a35a}.certificate-title{font-size:2rem;font-weight:800;color:#1a4f6e;margin:20px 0}.certificate-subtitle{font-size:1rem;color:#64748b;margin-bottom:30px;border-bottom:2px solid #c4a35a;display:inline-block;padding-bottom:5px}.certificate-student-name{font-size:1.8rem;font-weight:800;background:linear-gradient(135deg,#c4a35a,#a07d3a);-webkit-background-clip:text;-webkit-text-fill-color:transparent;margin:20px 0}.certificate-rank span{font-size:1.5rem;font-weight:800;color:gold}.certificate-percentage{font-size:1.4rem;font-weight:800;color:#1a4f6e;margin:15px 0}.certificate-date{margin-top:30px;padding-top:20px;border-top:1px dashed #cbd5e1}.certificate-signature{margin-top:30px;display:flex;justify-content:space-between;align-items:center;padding:0 20px}.certificate-signature hr{width:150px;margin:5px 0;border:1px solid #cbd5e1}@media print{.certificate-page{page-break-after:always}}</style></head><body>`; topStudentsList.forEach((s,i)=>{ const rank=i+1, rt=getRankText(rank); const sem=s.semester||'first', tp=getTotalPossible(sem); html+=`<div class="certificate-page"><div class="certificate"><div class="certificate-logo"><img src="logo.png" alt="شعار المعهد" onerror="this.style.display='none'"></div><div class="certificate-title"><i class="fas fa-award"></i> شهادة تقدير</div><div class="certificate-subtitle">معهد رعاية الضبعية الفني للتمريض</div><div class="certificate-body"><p>تُمنح هذه الشهادة للطالب/ة</p><div class="certificate-student-name">${escapeHtml(s.fullName)}</div><p>رقم الجلوس: <strong>${s.studentCode}</strong></p><div class="certificate-rank">🏆 <span>${rt}</span> 🏆</div><div class="certificate-percentage">⭐ بنسبة نجاح ${s.percentage.toFixed(1)}% ⭐</div><p>📊 المجموع الكلي: ${s.total} / ${tp}</p><p style="margin-top:15px;color:#666;">وذلك تقديراً لتفوقه وجهده المتميز خلال الفصل الدراسي،<br>ونتمنى له دوام النجاح والتفوق.</p></div><div class="certificate-date"><i class="far fa-calendar-alt"></i> التاريخ: ${cd}</div><div class="certificate-signature"><div><hr><p>مدير المعهد</p></div><div><hr><p>وكيل المعهد</p></div></div></div></div>`; }); html+=`<script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()};<\/script></body></html>`; pw.document.write(html); pw.document.close(); };
function updateTopStudentsAfterDataChange() { renderTopStudents(); }

// ====================== بدء التشغيل ======================
(async function init() { 
    if (await verifyAdminAccess()) { 
        renderNavbar(); renderAdminWelcomeMessage(); await loadInitialData(); await loadExamsList(); renderQuestionInputs(); window.toggleSubjects();
