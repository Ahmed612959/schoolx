...
// ====================== ✅ تحليل Excel (بدون صف - تربية دينية - تخطي المجموع) ======================
window.analyzeExcel = async () => { 
    ...
    reader.onload = async (e) => { 
        try {
            ...
            console.log(`📊 تم قراءة ${totalRows} صف`);
            ...
            for (let i = 1; i < rows.length; i++) { 
                ...
                // A=رقم جلوس, B=اسم, C=عربي, D=إنجليزي, E=علوم تطبيقية, F=طب باطنة, G=تمريض, H=حاسب آلي, I=تربية دينية, J=المجموع
                let subjects = [];
                ...
                if (row[8] !== undefined && row[8] !== '') subjects.push({ name: "الدين", grade: parseFloat(row[8]) || 0 }); // I = تربية دينية
                // row[9] = المجموع ← نتخطاه
            ...
            if (studentsData.length > 0) {
                try {
                    ...
                    const r = await fetch(`${BASE_URL}/api/upload-grades`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-CSRF-Token': csrfToken }, credentials: 'include', body: JSON.stringify({ students: studentsData }) });
                    const d = await r.json(); if (!r.ok) throw new Error(d.error || 'فشل الرفع');
                    log(`✅ تم إرسال ${studentsData.length} طالب للسيرفر`, 'success');
                } catch (err) { errorCount++; log(`❌ فشل: ${err.message}`, 'error'); }
            }
            
            up(totalRows, totalRows); if (pb) { pb.style.width = '100%'; pb.style.background = 'linear-gradient(90deg,#27ae60,#2ecc71)'; }
            allStudents = await getFromServer('/api/admin/students'); studentsWithGrades = getStudentsWithGrades(allStudents); renderResults(); renderStats();
            
            let msg = ''; if (updatedCount > 0) msg += `🔄 تم تحديث ${updatedCount} طالب. `; if (addedCount > 0) msg += `➕ تم إضافة ${addedCount} طالب جديد. `; if (skippedCount > 0) msg += `⏭️ تخطي ${skippedCount} صف. `; if (errorCount > 0) msg += `❌ فشل ${errorCount}. `;
            if (pt) { pt.innerHTML = `✅ ${msg}`; pt.style.color = '#27ae60'; pt.style.fontWeight = 'bold'; }
            log(`🎉 ${msg}`, 'success'); showToast(msg || '✅ تم الانتهاء', 'success');
