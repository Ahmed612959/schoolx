// Home.js - النسخة الكاملة النهائية (نظام حضور محسن + بحث متعدد المستويات)
...
function getSubjectConfig(semester) { return semester === 'first' ? SUBJECTS_CONFIG_FIRST : SUBJECTS_CONFIG_SECOND; }
function getOrderedSubjects(semester) { return semester === 'first' ? ORDERED_SUBJECTS_FIRST : ORDERED_SUBJECTS_SECOND; }
function getTotalPossible(semester) { return semester === 'first' ? TOTAL_POSSIBLE_FIRST : TOTAL_POSSIBLE_SECOND; }
function getSemesterName(semester) { return semester === 'first' ? 'الترم الأول' : 'الترم الثاني'; }
...
function calculateStudentTotal(student) {
    ...
    let t = 0;
    student.subjects.forEach(s => { const normalizedName = normalizeSubjectName(s.name); const c = config[normalizedName]; if (c && !c.isExtra) t += s.grade || 0; });
    return t;
}
...
async function fetchAllStudents() {
    try { const response = await apiRequest(`/api/admin/students`); if (response.ok) return await response.json(); return []; } catch (error) { console.error('خطأ في جلب الطلاب:', error); return []; }
}

async function fetchStudentByCode(studentCode) {
    try { const allStudents = await fetchAllStudents(); return allStudents.find(s => s.studentCode === studentCode) || null; } catch (error) { console.error('خطأ في جلب الطالب:', error); return null; }
}

async function fetchViolationsForStudent(studentId) {
    try { const response = await apiRequest(`/api/violations/student/${encodeURIComponent(studentId)}`); if (response.ok) return await response.json(); return []; } catch (error) { console.error('خطأ في جلب المخالفات:', error); return []; }
}
...
async function loadNotifications() {
    try {
        ...
        if (response.ok) {
            ...
            if (tableBody) {
                tableBody.innerHTML = '';
                if (!notifications.length) { tableBody.innerHTML = '<tr><td colspan="2">لا توجد إشعارات حاليًا</td></tr>'; return; }
                notifications.forEach(n => { const row = document.createElement('tr'); row.innerHTML = `<td>${n.text || 'إشعار بدون نص'}</td><td>${n.date || 'غير محدد'}</td>`; tableBody.appendChild(row); });
            }
        }
    } catch (error) { console.error('خطأ في تحميل الإشعارات:', error); }
}

// ====================== ✅ الحضور - الإصدار المتوافق مع السيرفر ======================
...
async function fetchAttendanceStats(studentCode, force = false) {
    ...
    try {
        ...
        return attendanceStats || { present: 0, absent: 0, late: 0, total: 0, percentage: '0', lastPresentDate: null, lastAbsentDate: null, records: [], hasRecords: false };
    }
}

function formatDate(dateStr) {
    if (!dateStr) return null;
    try { const parts = dateStr.split('-'); return parts.length === 3 ? `${parts[2]}/${parts[1]}/${parts[0]}` : dateStr; } catch { return dateStr; }
}
...
function renderAttendanceStats() {
    ...
    if (!user || user.type !== 'student') { section.style.display = 'none'; return; }
    ...
    if (!hasRecords) {
        if (noMsgDiv) { noMsgDiv.style.display = 'block'; noMsgDiv.innerHTML = '📌 لم يتم تسجيل أي حضور أو غياب لك حتى الآن.'; }
    ...
    if (noMsgDiv) { noMsgDiv.style.display = 'none'; noMsgDiv.innerHTML = ''; }
...
async function renderDashboard() {
    ...
    if (!student) { dashboard.style.display = 'none'; return; }
    ...
    if (student.subjects && student.subjects.length > 0) {
        ...
        if (!religionDiv) { religionDiv = document.createElement('div'); religionDiv.id = 'religionDisplay'; religionDiv.style.cssText = 'margin-top: 10px; padding: 8px; background: #f0f0f0; border-radius: 8px; text-align: center;'; const statsDiv = document.querySelector('.stats'); if (statsDiv) statsDiv.appendChild(religionDiv); }
...
function renderStudentResult(student, studentViolations, resultBody, violationsBody, searchName = '', searchMethod = '') {
    ...
    // ✅ رسالة توضح طريقة البحث المستخدمة
    let searchMethodMessage = '';
    if (searchMethod === 'code_only') {
        searchMethodMessage = `<div style="background: #e3f2fd; padding: 8px; border-radius: 8px; margin-bottom: 10px; text-align: center; font-size: 0.85em;">
            💡 تم العثور على الطالب برقم الجلوس فقط. الاسم المسجل: <strong>${escapeHtml(student.fullName)}</strong>
        </div>`;
    } else if (searchMethod === 'name_only') {
        searchMethodMessage = `<div style="background: #fff3e0; padding: 8px; border-radius: 8px; margin-bottom: 10px; text-align: center; font-size: 0.85em;">
            💡 تم العثور على الطالب بالاسم فقط (رقم الجلوس غير مطابق). رقم الجلوس المسجل: <strong>${student.studentCode}</strong>
        </div>`;
    }
    ...
    orderedSubjects.forEach(subjName => {
        ...
        if (subjectConfig?.isExtra) { subjectGrades.push({ name: `${subjName} (خارج المجموع)`, grade: grade, max: subjectConfig.max, isExtra: true }); }
        else { subjectGrades.push({ name: subjName, grade: grade, max: subjectConfig?.max || 100, isExtra: false }); total += grade; }
    });
    ...
    if (violationsBody) {
        if (studentViolations && studentViolations.length) { violationsBody.innerHTML = studentViolations.map(v => `<tr><td>${v.type === 'warning' ? '⚠️ إنذار' : '🚫 مخالفة'}</td><td>${v.reason || '-'}</td><td>${v.penalty || '-'}</td><td>${v.parentSummons ? '✅ نعم' : '❌ لا'}</td><td>${v.date || '-'}</td></tr>`).join(''); }
...
function renderMultipleResults(results, resultBody, violationsBody, searchMethod = '') {
    ...
    if (results.length === 1) { fetchViolationsForStudent(results[0].studentCode).then(violations => { renderStudentResult(results[0], violations, resultBody, violationsBody, '', searchMethod); }); return; }
    
    let html = `<tr><td colspan="4" style="background: #e3f2fd; padding: 10px; text-align: center; color: #1a2526;">
        🔍 تم العثور على <strong>${results.length}</strong> نتائج. اضغط على أي صف للتفاصيل.
    </td></tr>`;
    ...
    results.forEach(student => {
        ...
        html += `<tr style="cursor: pointer;" onclick="viewStudentDetail('${student.studentCode}')"><td><strong>${escapeHtml(student.fullName)}</strong></td><td>${student.studentCode}<br><small style="color: #d4af37;">${semesterName}</small></td><td>${calculateStudentTotal(student)} / ${getTotalPossible(detectedSemester)}</td><td class="${pClass}">${percentage.toFixed(1)}%</td></tr>`;
    });
    resultBody.innerHTML = html;
    if (violationsBody) { violationsBody.innerHTML = '<tr><td colspan="5" style="color: #d4af37;">🔍 تم العثور على عدة نتائج. اضغط على أي صف للتفاصيل.</td></tr>'; }
...
window.viewStudentDetail = async function(studentCode) {
    ...
    if (student) { const violations = await fetchViolationsForStudent(studentCode); renderStudentResult(student, violations, document.getElementById('result-table-body'), document.getElementById('violations-table-body')); window.scrollTo({ top: document.querySelector('.result-table')?.offsetTop || 0, behavior: 'smooth' }); }
};
...
async function searchStudentsByNameAndCode(name, studentCode) {
    try {
        ...
        // المستوى الأول: البحث بالاسم ورقم الجلوس معًا
        let results = allStudents.filter(s => s.studentCode === studentCode);
        if (name) {
...
// ✅ دوال البحث المنفردة للمستويات التالية
async function searchByCodeOnly(studentCode) {
    const allStudents = await fetchAllStudents();
    if (!allStudents.length) return [];
    return allStudents.filter(s => s.studentCode === studentCode || s.studentCode.includes(studentCode));
}
...
async function searchByNameOnly(name) {
    ...
    let results = allStudents.filter(s => {
        ...
        const allPartsFound = searchParts.every(part => studentName.includes(part));
        const similarity = calculateSimilarity(name, s.fullName || '');
        return allPartsFound || similarity >= 60;
    });
    results.sort((a, b) => { const simA = calculateSimilarity(name, a.fullName || ''); const simB = calculateSimilarity(name, b.fullName || ''); return simB - simA; });
...
function setupSearchForm() {
    ...
    searchInputs.forEach(input => { input.addEventListener('keypress', function(e) { if (e.key === 'Enter') { e.preventDefault(); form.dispatchEvent(new Event('submit')); } }); });
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            // ====== المستوى الأول: بحث كامل (اسم + رقم جلوس) ======
            ...
            if (results.length > 0) {
                ...
                renderStudentResult(results[0], violations, resultBody, violationsBody, name, results.length === 1 && calculateSimilarity(name, results[0].fullName || '') < 80 ? 'code_only' : '');
                ...
                if (similarity >= 90) { showToast(`✅ تم العثور على الطالب: ${results[0].fullName} - ${semesterName}`, 'success'); }
                else { showToast(`✅ تم العثور على الطالب برقم الجلوس: ${results[0].fullName} - ${semesterName}`, 'warning'); }
                return;
            }
            
            // ====== المستوى الثاني: البحث برقم الجلوس فقط ======
            showToast('🔍 لم يتم العثور على تطابق كامل... جاري التحقق برقم الجلوس فقط...', 'info');
            const codeResults = await searchByCodeOnly(studentCode);
            
            if (codeResults.length === 1) {
                // ✅ وجد طالب واحد بنفس رقم الجلوس
                const violations = await fetchViolationsForStudent(codeResults[0].studentCode);
                renderStudentResult(codeResults[0], violations, resultBody, violationsBody, name, 'code_only');
                const detectedSemester = codeResults[0].semester || detectSemester(codeResults[0].subjects) || 'first';
                const semesterName = getSemesterName(detectedSemester);
                showToast(`✅ تم العثور على الطالب برقم الجلوس: ${codeResults[0].fullName} - ${semesterName} (الاسم المدخل مختلف)`, 'warning');
                return;
            } else if (codeResults.length > 1) {
                // ✅ وجد عدة طلاب بنفس رقم الجلوس
                renderMultipleResults(codeResults, resultBody, violationsBody, 'code_only');
                showToast(`✅ تم العثور على ${codeResults.length} طلاب بنفس رقم الجلوس`, 'info');
                return;
            }
            
            // ====== المستوى الثالث: البحث بالاسم فقط ======
            showToast('🔍 جاري البحث بالاسم فقط...', 'info');
            const nameResults = await searchByNameOnly(name);
            
            if (nameResults.length === 1) {
                // ✅ وجد طالب واحد بنفس الاسم
                const violations = await fetchViolationsForStudent(nameResults[0].studentCode);
                renderStudentResult(nameResults[0], violations, resultBody, violationsBody, name, 'name_only');
                const detectedSemester = nameResults[0].semester || detectSemester(nameResults[0].subjects) || 'first';
                const semesterName = getSemesterName(detectedSemester);
                showToast(`✅ تم العثور على الطالب بالاسم: ${nameResults[0].fullName} - رقم الجلوس: ${nameResults[0].studentCode} - ${semesterName}`, 'warning');
                return;
            } else if (nameResults.length > 1) {
                // ✅ وجد عدة طلاب بنفس الاسم
                renderMultipleResults(nameResults, resultBody, violationsBody, 'name_only');
                showToast(`✅ تم العثور على ${nameResults.length} طلاب بالاسم. اختر الطالب الصحيح`, 'info');
                return;
            }
            
            // ====== كل المستويات فشلت ======
            resultBody.innerHTML = `<tr><td colspan="4" style="text-align: center; padding: 20px;">
                <div style="color: #dc3545; font-size: 1.1em; margin-bottom: 10px;">❌ لم يتم العثور على أي طالب</div>
                <div style="color: #666; font-size: 0.85em;">
                    تأكد من:<br>
                    • صحة كتابة الاسم ورقم الجلوس<br>
                    • عدم وجود مسافات زائدة<br>
                    • أن الطالب مسجل في النظام<br>
                </div>
            </td></tr>`;
...
// ====================== باقي الدوال (الناف بار، الترحيب، الجلسة، المرشد، البوت) ======================
function renderNavbar() {
    const user = getLoggedInUser(); const navBar = document.getElementById('nav-bar'); if (!navBar) return;
    const links = [{ href: 'index.html', icon: 'fa-solid fa-house', title: 'الرئيسية' },{ href: 'Home.html', icon: 'fa-solid fa-chart-simple', title: 'النتائج' },{ href: 'profile.html', icon: 'fa-solid fa-user', title: 'الملف الشخصي' },{ href: 'search-monthly.html', icon: 'fa-solid fa-magnifying-glass', title: 'نتيجة الشهري' },{ href: 'First-Gards.html', icon: 'fa-solid fa-graduation-cap', title: 'نتيجة الصف الاول' },{ href: 'exams.html', icon: 'fa-solid fa-book-open', title: 'الاختبارات' },{ href: 'file-library.html', icon: 'fas fa-folder-open', title: 'المكتبة' },{ href: 'developer.html', icon: 'fa-solid fa-microchip', title: 'عن المطور' }];
    if (user?.type === 'admin') links.push({ href: 'admin.html', icon: 'fas fa-cogs', title: 'لوحة التحكم' });
...
async function init() {
    ...
    setInterval(async () => { try { await fetch(`${BASE_URL}/api/refresh-token`, { method: 'POST', credentials: 'include' }); } catch (e) {} }, 55 * 60 * 1000);
    setTimeout(() => { if (document.getElementById('liveBot')) { window.smartBot = new SmartAssistantBot(); } }, 800);
}
